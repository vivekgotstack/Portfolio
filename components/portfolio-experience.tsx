...
function SystemCore() {
  return (
    <div className="relative mx-auto aspect-square w-full max-w-130 select-none" aria-hidden="true">
      ...
      <span className="absolute left-[3%] top-[28%] rounded border border-white/10 bg-black/70 px-3 py-2 font-sans text-[9px] text-white/50 backdrop-blur">STATUS: SHIPPING</span>
      <span className="absolute bottom-[18%] right-0 rounded border border-white/10 bg-black/70 px-3 py-2 font-sans text-[9px] text-[#c7ff38] backdrop-blur">UPTIME 99.9%</span>
      <span className="absolute right-[4%] top-[20%] font-sans text-[9px] text-white/30">01° DEV_CORE</span>
    ...
    return (
      <div className="relative min-h-85 overflow-hidden bg-[#111410] sm:min-h-105">
        ...
        <div className="absolute bottom-0 left-0 flex w-full items-end justify-between p-7 sm:p-9">
          <div><p className="font-sans text-[9px] uppercase tracking-[.2em] text-[#c7ff38]">Production showcase</p><p className="mt-2 text-xl font-medium">ModaStitch product film</p></div>
          <span className="rounded-full border border-white/15 bg-black/35 px-3 py-2 font-sans text-[8px] uppercase tracking-wider text-white/60 backdrop-blur">Live</span>
        </div>
      </div>
    );
  }
  ...
  return (
    <div ref={container} className="min-h-screen overflow-hidden bg-[#080a08] text-[#f2f4eb] selection:bg-[#c7ff38] selection:text-black">
      ...
      <nav className="fixed left-1/2 top-4 z-50 flex w-[calc(100%-24px)] max-w-6xl -translate-x-1/2 items-center justify-between rounded-full border border-white/10 bg-black/55 px-4 py-3 backdrop-blur-xl sm:px-5">
        ...
        <div className="hidden items-center gap-6 font-sans text-[10px] uppercase tracking-[.14em] text-white/50 sm:flex"><a href="#work" className="transition hover:text-white">Work</a><a href="#about" className="transition hover:text-white">About</a><a href="#contact" className="transition hover:text-white">Contact</a></div>
        <div className="flex items-center gap-2"><PetCompanion /><a href="#contact" className="hidden items-center gap-2 rounded-full border border-[#c7ff38]/30 px-3 py-2 font-sans text-[9px] uppercase tracking-wider text-[#c7ff38] md:flex"><span className="h-1.5 w-1.5 animate-pulse rounded-full bg-[#c7ff38]" /> Available</a></div>
      </nav>
      ...
      <main className="relative z-10">
        ...
        <div className="border-y border-white/10 bg-[#c7ff38] py-3 text-black">
          <div className="flex w-max animate-[marquee_28s_linear_infinite] gap-8 whitespace-nowrap font-sans text-[10px] font-bold tracking-[.16em]">
                ...
                <Reveal key={project.title}>
                  <a href={project.href} target="_blank" rel="noreferrer" className="group grid overflow-hidden rounded-4xl border border-white/10 bg-white/2.5 transition duration-500 hover:border-white/25 hover:bg-white/4.5 lg:grid-cols-[.82fr_1.18fr]">
                    <div className="flex min-h-97.5 flex-col justify-between p-7 sm:p-10 lg:min-h-115">
                      <div>
                        <div className="mb-10 flex items-center justify-between">
                          <div className="flex items-center gap-3">
                            ...
                            <span className="font-sans text-[10px] text-white/30">{project.n}</span>
                          </div>
                        ...
                        <p className="mb-3 font-sans text-[9px] uppercase tracking-[.18em]" style={{ color: project.color }}>{project.type}</p>
                      ...
                      <div className="mt-10 flex flex-wrap gap-2">{project.tags.map((tag) => <span key={tag} className="rounded-full border border-white/10 px-3 py-1.5 font-sans text-[8px] uppercase tracking-wider text-white/40">{tag}</span>)}</div>
                    </div>
            ...
            {experience.map((job, i) => (
              <Reveal key={job.company}>
                <div className="relative grid gap-5 border-t border-white/10 py-10 sm:grid-cols-[1fr_1fr]">
                  ...
                  <div><p className="font-sans text-[9px] text-[#c7ff38]">0{i + 1} / {job.period}</p><h3 className="mt-3 text-2xl font-medium">{job.company}</h3><p className="text-sm text-white/35">{job.role}</p></div>
      ...
      <footer className="relative z-10 border-t border-white/10 px-5 py-8 lg:px-12"><div className="mx-auto flex max-w-336 flex-col gap-5 font-sans text-[9px] uppercase tracking-[.15em] text-white/30 sm:flex-row sm:items-center sm:justify-between"><p>© 2026 Vivek Nigam / Engineered end to end</p><div className="flex gap-5"><a href="https://github.com/vivekgotstack" target="_blank" rel="noreferrer" className="flex items-center gap-2 hover:text-white"><Github className="h-3.5 w-3.5" /> GitHub</a><a href="mailto:vivekgotstack@gmail.com" className="flex items-center gap-2 hover:text-white"><Mail className="h-3.5 w-3.5" /> Email</a><a href="https://www.linkedin.com/in/randomvivek/" target="_blank" rel="noreferrer" className="flex items-center gap-2 hover:text-white"><Linkedin className="h-3.5 w-3.5" /> LinkedIn</a><span className="hidden w-14.5 shrink-0 sm:block" aria-hidden="true" /></div></div></footer>
